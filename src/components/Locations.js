...
import useAutocompleteCities from "../hooks/useAutocompleteCities";
import useLocationLatLng from "../hooks/useLocationLatLng";
import {
      ...
      try {
       ...
       // dispatch(addLatLng({lat:position.coords.latitude, lng:position.coordinates.longitude}));
        ...
        if (error.code === error.PERMISSION_DENIED) {
          ...
          alert("Geolocation information is not available. Please check your network connection and try again.");
        } else if (error.code === error.TIMEOUT) {
          alert("Geolocation request timed out. Please try again.");
        } else {
          alert("An error occurred while fetching your location. Please try again later.");
        }
      }
    } else {
